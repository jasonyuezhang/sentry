...
- stop_colocated -- "true" or "false" -- When true, skip SUNIONSTORE merges and colocated key expire (phase 3).
...
-- When stop_colocated is true, SUNIONSTORE is skipped but we still need to merge
-- ic/ibc counters from child colocated keys into the segment root (the Lua always
-- writes ic/ibc to the colocated set_key, even in phase 3).
