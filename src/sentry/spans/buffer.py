...
class SpansBuffer:
    ...
    def _load_segment_data(self, segment_keys: list[SegmentKey]) -> dict[SegmentKey, list[bytes]]:
        ...
        def _add_spans(segment_key: SegmentKey, raw_data: bytes) -> bool:
            ...
            sizes[segment_key] = sizes.get(segment_key, 0) + sum(len(span) for span in decompressed)
            if sizes[segment_key] > max_segment_bytes:
                ...
                payloads.pop(segment_key, None)
                sizes.pop(segment_key, None)
                dropped_segments.add(segment_key)
                return False
        ...
        if dropped_segments and distribute_read:
            with self.client.pipeline(transaction=False) as p:
