...
def test_get_hint_for_frame(
    ...
    assert (
        # We have to ignore input types here because it doesn't recognize the dummy rust frame type
        _get_hint_for_frame(variant_name, frame, frame_component, rust_frame, desired_hint_type, set())  # type: ignore[arg-type]
        == expected_result
    )
...
def test_adds_rule_source_to_stacktrace_rule_hints() -> None:
    ...
    assert (
        _get_hint_for_frame(
            ...
            frame,
            frame_component,
            built_in_rule_rust_frame,  # type: ignore[arg-type] mock doesn't match rust type
            "in-app",
            custom_rules,
        )
    ...
    assert (
        _get_hint_for_frame(
            ...
            frame,
            frame_component,
            custom_rule_rust_frame,  # type: ignore[arg-type] mock doesn't match rust type
            "in-app",
            custom_rules,
        )
